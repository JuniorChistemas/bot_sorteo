...
db.prepare(`CREATE TABLE IF NOT EXISTS participation (
...
export function registerParticipation(phoneNumber: string, number: number){
    db.prepare('INSERT INTO participation (phone_number, number, timestamp) VALUES (?, ?, ?)').run(phoneNumber, number, today());
}
